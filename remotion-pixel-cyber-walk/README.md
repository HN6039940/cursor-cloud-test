...
| 背景色 | `#0B0B12` |
| パララックス | far **0.15×** / mid **0.45×** / ground **1×** / キャラ **0** / 雨 **0.8×** + 縦落下 |
| 地面速度 | 8 px/frame（15 秒でタイル 3 周、ループで位相が戻る） |
...
歩行は 18 frames 周期で、360 frames にちょうど 20 周入ります。地面は幅 960 の 3 周分進むので、映像の先頭と末尾で水たまりの位相が揃います。中景は固定比 0.45× のまま、末尾で 16 px だけずれます。遠景と雨の横位置は、この尺だとタイル幅の公倍数に乗りません。再生中のスクロールは途切れません。
